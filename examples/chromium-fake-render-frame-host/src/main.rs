...
fn main() {
    ...
    let create_params =
        ffi::content::CreateParams::new(&"silly-frame".into_cpp()).within_unique_ptr();
...

    {
        ...
        // Here the handle goes out of scope, and its destructor unregisters
        // the observer from the WebContents. That deregistration had better
        // be real: the handle owns the observer's C++ peer, which dies along
        // with it, so a WebContents that hung on to a stale pointer would
        // call into freed memory the next time it announced anything.
    }

    // So let's make it announce something. A fresh handle, watching the same
    // RenderFrameHost - and it should be the only observer listening.
    let mut rfh_handle = RenderFrameHostHandle::from_id(c_int(3), c_int(0), frame.pin_mut());
    ...
    // Exactly one "a handle noticed..." line above, please. Two would mean the
    // WebContents was still talking to the handle we dropped.
    assert_eq!(
        render_frame_host::DELETION_NOTICES.load(std::sync::atomic::Ordering::Relaxed),
        1,
        "only the surviving handle should have been notified"
    );
    assert!(
        rfh_handle.try_borrow_mut().is_none(),
        "the surviving handle should have seen the RFH destroyed"
    );
    println!("The RenderFrameHost is gone, and the surviving handle knows it.");
}
